...
from visualization.rollout_metrics import run_removal


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run leave-one-step-out Removal Damage")
    ...
    parser.add_argument("--max-new-tokens-removal", type=int)
...
def main() -> None:
    ...
    decoding = DecodingConfig(
        ...
        max_new_tokens=choose(
            args.max_new_tokens_removal,
            config["decoding"]["removal_max_new_tokens"],
        ),
    )
    ...
    exclusions.write(output_root / "reports" / f"{run_name}_removal_exclusions.json")
    save_run_config(
        output_root / "configs" / f"{run_name}_removal.json",
        {
            "backbone": backbone,
            "adapters": None,
            "thinking_mode": False,
